...
// wip: 22 Jan 2023
// djrm, hackked to control Smits H6 horizontal & vertical lines independantley. Feb 9th 2023
// uses two potentiometers for input connected to A0 and A3
...
int Ver_IN = A3;
int Freq_IN = A4;
int potVal,potHor,potVer = 200;
int newVal,newHor,newVer;
byte phaseVal,phaseHor,phaseVer;
...
//  pinMode(enablePin, OUTPUT);      // sets the digital pin as output
  ...
  pinMode(Ver_IN, INPUT);
  pinMode(Hor_IN, INPUT);
...
void loop(){
  ...
  if( (potHor=analogRead(Hor_IN)) != newHor)
  {
    newHor=potHor;
    if(potHor>1020)potHor=1020;
    phaseHor=map(potHor,0,1020,0,255);
  }

  if( (potVer=analogRead(Ver_IN)) != newVer)
  {
    newVer=potVer;
    if(potVer>1020)potVer=1020;
    phaseVer=map(potVer,0,1020,0,255);
  }
...
void zerocrossing()          
{                   
   sigma = 0L;                      // sync generator to zero crossing
}
...
ISR(TIMER2_OVF_vect) {
  ...
  byte idx,idxHor,idxVer;                       // read value fron ROM sine table and send to PWM DAC
  byte amp;
...
// to feed into S1 and S2, S3 being common ground.
 #if 1
  idx = 128 | phaseHor;                             // rotation angle (0 o 255)
  phaseA = 2 * pgm_read_byte_near(sine256 + idx);   // sin rotation angle is scale factor

  if(phaseHor < 64 || phaseHor > 191)               // invert when needed
...
//------------------------------------------------------------------------------------------
  ...
  idx = 128 | phaseVer;                        // shifted rotation angle 
  phaseB = 2 * pgm_read_byte_near(sine256 + idx);

  if(phaseVer < 64 || phaseVer > 191)              
